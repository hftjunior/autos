...
@section('content')
    <page-gridsearch 
        ...
        v-bind:columns="['#','Marca']"
        v-bind:items="{{ json_encode($manufacturers) }}"
        detail="/vehicle/manufacturers/" edit="/vehicle/manufacturers/" del="/vehicle/manufacturers/" token="{{ csrf_token() }}" modal="1">
    <span slot="btns">
        @can('create_manufacturer', User::class)
    ...
    <modal name="creationForm" title="{{ $title }}">
        <page-form id="formCreation" css="" action="{{ route('manufacturers.store') }}" method="post" enctype="multipart/form-data" token="{{ csrf_token() }}">
            <div class="box-body">
                <div class="form-group">
                    <label for="manufacturer">Marca</label>
                    <input type="text" class="form-control" name="manufacturer" id="manufacturer" paceholder="Marca" value="{{old('manufacturer')}}">
                </div>
            </div>
        </page-form>
        <span slot="btns">
    ...
    <modal name="editionForm" title="{{ $title }}">
        <page-form id="formEdition" css="" v-bind:action="'/vehicle/manufacturers/' + $store.state.item.id" method="put" enctype="multipart/form-data" token="{{ csrf_token() }}">
            <div class="box-body">
                <div class="form-group">
                    <label for="manufacturer">Marca</label>
                    <input type="text" class="form-control" name="manufacturer" id="manufacturer" paceholder="Marca" v-model="$store.state.item.manufacturer">
                </div>
            </div>
        </page-form>
        <span slot="btns">
            @can('update_manufacturer', User::class)
