...
@section('content')
    <page-gridsearch 
        title="Combustíveis de Veículos"
        v-bind:columns="['#','Combustível']"
        v-bind:items="{{ json_encode($fuels) }}"
        detail="/vehicle/fuels/" edit="/vehicle/fuels/" del="/vehicle/fuels/" token="{{ csrf_token() }}" modal="1">
    <span slot="btns">
        @can('create_fuel', User::class)
    ...
    <modal name="creationForm" title="{{ $title }}">
        <page-form id="formCreation" css="" action="{{ route('fuels.store') }}" method="post" enctype="multipart/form-data" token="{{ csrf_token() }}">
            <div class="box-body">
                <div class="form-group">
                    <label for="fuel">Combustível</label>
                    <input type="text" class="form-control" name="fuel" id="fuel" paceholder="Combustível" value="{{old('fuel')}}">
                </div>
            </div>
        </page-form>
        <span slot="btns">
    ...
    <modal name="editionForm" title="{{ $title }}">
        <page-form id="formEdition" css="" v-bind:action="'/vehicle/fuels/' + $store.state.item.id" method="put" enctype="multipart/form-data" token="{{ csrf_token() }}">
            <div class="box-body">
                <div class="form-group">
                    <label for="fuel">Combustível</label>
                    <input type="text" class="form-control" name="fuel" id="fuel" paceholder="Combustível" v-model="$store.state.item.fuel">
                </div>
            </div>
        </page-form>
        <span slot="btns">
            @can('update_fuel', User::class)
            ...
            <div class="info-box-content">
                <div class="row">
                    <div class="col-sm-4 border-right">   
                        ...
                        <span class="info-box-number">@{{ $store.state.item.fuel }}</span>
                    </div>
                </div>
            </div>
        </div>
